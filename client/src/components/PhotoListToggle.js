...
// const StyledPhotoListToggle = styled.div`
...
function PhotoListToggle(props) {
	if (props.photoListShow) {		
		...
		if (props.featureImgObj.index >= props.imagesArr.length-9) {
		...
		console.log(props.imagesArr.slice(start, end))
			return (
				...
				<PhotoList imagesArr={props.imagesArr.slice(start, end)} changeFeatureImg={props.changeFeatureImg} />
				</div>
				)
		} else {
			return (
				<div onClick={props.togglePhotoList}>
				<span> Show photo list 
